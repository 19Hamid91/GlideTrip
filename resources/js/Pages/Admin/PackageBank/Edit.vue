...
const bank = ref(page.props.bank || { data: [], links: [] });

// Form handling with Inertia
const form = useForm({
    _method: 'patch',
    bank_name: bank.value.bank_name,
    bank_account_name: bank.value.bank_account_name,
    bank_account_number: bank.value.bank_account_number,
    logo: null
});

const submitForm = () => {
    form.post(route('admin.package_banks.update', { package_bank: bank.value.id }), {
        onFinish: () => form.reset()
    });
};
</script>
